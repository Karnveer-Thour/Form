function Userform({ handleInvalidinput, data, setData, emailValidator }) {
  ...
  const handleChange = (e) => {
    ...
    if (rawValue.length < data.PersonNumber.length) {
      setData({ ...data, PersonNumber: rawValue });
      return;
    } else {
      const formatted = formatIndianPhoneNumber(rawValue);

      setData({ ...data, PersonNumber: formatted });
    }
  };
  return (
    <div className="mb-3">
      <div className="mt-4 d-flex flex-column justify-content-start align-items-start w-100">
        <p
          style={{
            fontSize: "1rem",
      ...
      <div className="usercontainer mb-3 d-flex justify-content-between flex-row align-items-start flex-wrap w-100">
        <div className="ms-5 personName" style={{ width: "40%" }}>
          ...
          />
          <p
            className=" mb-0 mt-0 fs-5"
            style={{ color: "red", display: "none", textAlign: "left" }}
          >
            Enter a valid Name
          </p>
        </div>
